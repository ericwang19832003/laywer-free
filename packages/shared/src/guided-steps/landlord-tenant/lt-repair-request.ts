...
export function createLtRepairRequestConfig(state?: string): GuidedStepConfig {
  const lt = getLandlordTenantInfo(state)

  return {
    title: 'Request Repairs from Your Landlord',
    reassurance: 'The law requires landlords to maintain habitable conditions. You have the right to request repairs.',
    ...
    questions: [
      ...
      },
      {
        ...
        helpText: 'Visual evidence strengthens your case significantly.',
      },
      {
    ...
    generateSummary(answers) {
      ...
      if (answers.prior_notice === 'yes') {
        ...
        if (answers.notice_method === 'written_letter') {
          ...
          items.push({
            status: 'needed',
            text: `Send written notice (letter or email) to your landlord. Verbal notice alone is not sufficient under ${lt.repairStatuteShort}.`,
          })
        }
      } else {
        items.push({
          status: 'needed',
          text: `Send written notice to your landlord describing the repair needed. This is required under ${lt.repairStatute}.`,
        })
      }
...
export const ltRepairRequestConfig = createLtRepairRequestConfig()
