...
export function createScEvidenceRulesConfig(state?: string): GuidedStepConfig {
  const sc = getSmallClaimsInfo(state)

  return {
    title: 'Evidence Rules in Small Claims Court',
    reassurance: `${sc.courtAbbrev} has relaxed evidence rules. You don't need to be a lawyer to present your case.`,

    questions: [
      {
        id: 'know_relaxed_rules',
        type: 'yes_no',
        prompt: `Do you know how evidence rules differ in ${sc.courtAbbrev}?`,
      },
      {
        id: 'relaxed_rules_info',
        type: 'info',
        prompt: `${sc.courtAbbrev} uses relaxed evidence rules. Hearsay is more acceptable than in other courts, business records are easier to admit, and the judge has wide discretion on what to consider. You don't need to follow the formal rules of evidence.`,
        acknowledgeLabel: "I understand — relaxed evidence rules apply",
      ...
      },
      {
        ...
        acknowledgeLabel: "I'll bring all of this evidence",
      ...
      },
      {
        ...
        type: 'info',
        prompt: `For a car accident claim, bring: (1) The police report. (2) Photos of the damage to your vehicle. (3) At least one repair estimate (preferably 2-3). (4) Rental car receipts if applicable. (5) Insurance correspondence. (6) Medical bills if claiming injury (must be under ${sc.limit} total).`,
        acknowledgeLabel: "I'll bring all of this evidence",
      ...
      },
      {
        ...
        type: 'info',
        prompt: "For a consumer dispute, bring: (1) Your receipt or proof of purchase. (2) The product listing, advertisement, or description showing what was promised. (3) Photos of the defective product or unfinished work. (4) Communications with the seller/business. (5) Evidence of your refund request and their response.",
        acknowledgeLabel: "I'll bring all of this evidence",
      ...
      },
      {
        ...
        acknowledgeLabel: "I understand how to present evidence",
      ...
      },
      {
        id: 'organize_info',
        type: 'info',
        prompt: "Number each piece of evidence as an exhibit (Exhibit 1, Exhibit 2, etc.) in the order you'll present them. Make 3 copies of everything: one for you, one for the judge, and one for the other side. Use a folder or binder to keep everything organized.",
        acknowledgeLabel: "I'll number exhibits and make 3 copies",
      ...
      },
      {
        id: 'witness_info',
        type: 'info',
        prompt: "In small claims court, written witness statements (affidavits) are accepted even if the witness can't attend the hearing. Have your witness write a signed, dated statement describing what they saw or know. If possible, get it notarized. A witness who appears in person is more persuasive, but an affidavit is better than nothing.",
        acknowledgeLabel: "I'll get affidavits from my witnesses",
        showIf: (answers) => answers.have_witnesses === 'yes',
      },
      {
        id: 'no_witness_info',
        type: 'info',
        prompt: "You don't need witnesses to win. Many small claims cases are won on documents alone. Focus on your paper trail: contracts, receipts, photos, and communications.",
        acknowledgeLabel: "I'll focus on my document trail",
    ...
    generateSummary(answers) {
      ...
      if (answers.know_relaxed_rules === 'yes') {
        items.push({ status: 'done', text: `Understands ${sc.courtAbbrev} relaxed evidence rules.` })
      } else {
        items.push({
          status: 'info',
          text: `${sc.courtAbbrev} uses relaxed evidence rules. Hearsay is more acceptable and business records are easier to admit.`,
        })
      }
      ...
      if (answers.have_witnesses === 'yes') {
        items.push({
          status: 'info',
          text: 'Get written affidavits from witnesses. In-person testimony is stronger, but signed statements are accepted.',
        })
      }
...
export const scEvidenceRulesConfig = createScEvidenceRulesConfig()
