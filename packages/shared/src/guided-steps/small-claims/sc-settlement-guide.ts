...
export const scSettlementGuideConfig: GuidedStepConfig = {
  ...
  questions: [
    // When to settle
    {
      ...
      acknowledgeLabel: 'I understand the right time to settle →',
    },
    ...
    // How much to accept
    {
      ...
      acknowledgeLabel: 'I understand how to evaluate a settlement →',
      showIf: (answers) =>
    ...
    // Settlement agreement must-haves
    {
      ...
      type: 'multi_select',
      prompt: 'Does your draft settlement agreement include these required terms?',
      options: [
        { value: 'amount', label: 'Exact dollar amount to be paid' },
        { value: 'payment_deadline', label: 'Payment deadline (specific date)' },
        { value: 'release_claims', label: 'Release of all claims by both sides' },
        { value: 'dismissal', label: 'Agreement to dismiss the case' },
        { value: 'missed_payment', label: 'What happens if they miss the payment deadline' },
        { value: 'signatures', label: "Both parties' signatures and the date" },
      ],
    ...
    },
    {
      ...
      acknowledgeLabel: 'I will include an acceleration clause →',
    ...
    // Written agreement warning
    {
      ...
      acknowledgeLabel: 'I will not dismiss until I have payment →',
    },
    ...
    // Release form
    {
      ...
      acknowledgeLabel: 'I have my release language →',
    },
  ...
  generateSummary(answers) {
    ...
    // Agreement completeness check
    const agreedTerms = answers.agreement_contents
      ? answers.agreement_contents.split(',').filter((v: string) => v && v !== 'none')
      : []
    const requiredTerms = ['amount', 'payment_deadline', 'release_claims', 'dismissal', 'missed_payment', 'signatures']
    ...
    if (agreedTerms.length > 0 && missingTerms.length === 0) {
      ...
      items.push({ status: 'needed', text: `Settlement agreement is missing ${missingTerms.length} required term(s). Complete before signing.` })
    } else {
      items.push({
        status: 'needed',
