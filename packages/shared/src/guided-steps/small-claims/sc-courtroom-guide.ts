...
export function createScCourtroomGuideConfig(state?: string): GuidedStepConfig {
  const sc = getSmallClaimsInfo(state)

  return {
    title: 'What to Expect at Your Small Claims Hearing',
    reassurance: `${sc.courtAbbrev} hearings are short, informal, and designed for people without lawyers. You'll do better than you think.`,
    ...
    questions: [
      ...
      },
      {
        ...
        prompt: state === 'NY'
          ...
          : state === 'FL'
          ? 'Sample testimony: "Your Honor, I rented from the defendant at [address]. I moved out on [date] and provided my forwarding address. My deposit was $[amount]. Under Florida Statutes § 83.49, the landlord must return the deposit within 15 days if there are no deductions, or within 30 days if they claim a deduction — in which case they must first send written notice of the claim. More than [15/30] days have passed and I have received neither my deposit nor a proper written notice. Here is my lease and move-out confirmation."'
          : state === 'PA'
          ? 'Sample testimony: "Your Honor, I rented from the defendant at [address]. I moved out on [date] and provided my forwarding address in writing. My deposit was $[amount]. Under 68 P.S. § 250.512, the landlord must return the security deposit with an itemized list of deductions within 30 days of the tenant vacating and providing their forwarding address. It has been over 30 days and I have received neither my deposit nor an itemized statement. A landlord who wrongfully withholds a deposit forfeits the right to withhold any portion and may be liable for double the withheld amount. Here is my lease, move-out documentation, and forwarding address letter."'
...
export const scCourtroomGuideConfig = createScCourtroomGuideConfig()
