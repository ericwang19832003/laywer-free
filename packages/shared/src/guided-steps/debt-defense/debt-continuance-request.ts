...
export const debtContinuanceRequestConfig: GuidedStepConfig = {
  ...
  questions: [
    ...
    // Gather evidence — legal argument first, then evidence checklist
    {
      id: 'gather_evidence_legal_arg',
      type: 'info',
      acknowledgeLabel: 'Got it — what evidence do I need? →',
      showIf: (answers) => answers.continuance_reason === 'gather_evidence',
      prompt:
        'GATHERING EVIDENCE is one of the strongest grounds for a continuance. Texas Rule of Civil Procedure 252 allows continuances when a party needs more time to obtain evidence.\n\nIn your motion, explain:\n- What evidence you need\n- Why you have not been able to get it yet\n- How much time you need (be specific — "2 weeks" is better than "more time")\n\nExample: "I need to obtain bank records from [bank] showing payment history. I have requested these records but have not yet received them."',
    },
    {
      id: 'gather_evidence_checklist',
      type: 'multi_select',
      prompt: 'Which of these pieces of evidence do you have ready?',
      helpText:
        'Check everything you already have. Items not checked are what you still need — these go in your motion to explain why you need more time.',
      noneLabel: 'None ready yet',
      options: [
        { value: 'phone_records', label: 'Phone records (call logs from your carrier)' },
        { value: 'recordings', label: 'Call recordings (Texas is a one-party consent state)' },
        { value: 'collector_letters', label: 'Letters or mail from the collector' },
        { value: 'written_log', label: 'Written log of interactions (dates, times, what was said)' },
        { value: 'validation_request', label: 'Debt validation letter with certified mail receipt' },
        { value: 'witness_info', label: 'Witness information (name and what they heard or saw)' },
      ],
      showIf: (answers) => answers.continuance_reason === 'gather_evidence',
    },

    {
      ...
      type: 'info',
      acknowledgeLabel: 'Got the argument →',
    ...
    },
    {
      ...
      type: 'info',
      acknowledgeLabel: 'Got the steps →',
      prompt:
