...
export function createContractWaitForAnswerConfig(state?: string): GuidedStepConfig {
  const ct = getContractInfo(state)

  return {
    title: 'Wait for the Answer',
    reassurance: 'After the defendant is served with your contract lawsuit, they have a limited time to file an answer with the court. This step helps you track that deadline.',
    ...
    generateSummary(answers) {
      ...
      items.push({
        status: 'info',
        text: `The defendant has until ${ct.answerDeadline} to file an answer.`,
      })
...
export const contractWaitForAnswerConfig = createContractWaitForAnswerConfig()
