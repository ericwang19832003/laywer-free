...
export const reEarnestMoneyGuideConfig: GuidedStepConfig = {
  ...
  questions: [
    ...
    // How escrow works
    {
      ...
      acknowledgeLabel: 'I understand how escrow works →',
    },
    ...
    // Buyer backed out
    {
      ...
      acknowledgeLabel: 'I understand buyer default rules →',
    ...
    // Seller backed out
    {
      ...
      acknowledgeLabel: 'I understand my rights as a buyer →',
    ...
    // Failed contingency
    {
      ...
      acknowledgeLabel: 'I understand the contingency rules →',
      showIf: (answers) => answers.what_happened === 'failed_contingency',
    },

    // Mutual disagreement
    {
      id: 'mutual_disagreement_intro',
      type: 'info',
      prompt:
        'BOTH PARTIES DISAGREE:\n\nThis is the most common earnest money dispute scenario. The title company cannot release the money until both parties sign a mutual release form OR a court orders the release. Until one of those two things happens, the money stays frozen.',
      acknowledgeLabel: 'I understand →',
      showIf: (answers) => answers.what_happened === 'mutual_disagreement',
    },
    {
      id: 'mutual_disagreement_steps',
      type: 'info',
      prompt:
        'YOUR NEXT STEPS WHEN BOTH PARTIES DISAGREE:\n\n1. DEMAND LETTER: Send a written demand to the other party explaining why you are entitled to the earnest money. Request they sign the mutual release form within 10-15 days.\n2. MEDIATION: Many purchase agreements require mediation before filing suit. Check your contract — this step may be mandatory.\n3. INTERPLEADER: If neither party budges, the title company may deposit the money with the court and let the judge decide. This relieves the title company of liability.\n4. LAWSUIT: File suit for breach of contract and recovery of the earnest money if all else fails.',
      acknowledgeLabel: 'I know the steps to break the deadlock →',
      showIf: (answers) => answers.what_happened === 'mutual_disagreement',
    },
    ...
    // Title company holding
    {
      ...
      acknowledgeLabel: 'I understand the title company\'s role →',
    ...
    },
    {
      ...
      acknowledgeLabel: 'I understand option period termination rights →',
    ...
    },
    {
      ...
      acknowledgeLabel: 'I know what to include in my demand letter →',
      showIf: (answers) => answers.sent_demand === 'no',
    },

    // Dispute resolution — which path
    {
      id: 'resolution_path',
      type: 'single_choice',
      prompt: 'Which resolution path are you considering?',
      helpText:
        'Select the approach that best fits where you are in the dispute. You can always escalate to a later option if an earlier one fails.',
      options: [
        { value: 'mutual_release', label: 'Mutual release — ask the other party to sign the TREC release form' },
        { value: 'mediation', label: 'Mediation — use a neutral third party to negotiate' },
        { value: 'small_claims', label: 'Small claims (JP Court) — earnest money is under $20,000' },
        { value: 'district_court', label: 'District Court — larger amount or seeking additional damages' },
      ],
    },
    {
      id: 'mutual_release_detail',
      type: 'info',
      prompt:
        'MUTUAL RELEASE FORM:\nThe simplest path — both parties sign the TREC Release of Earnest Money form and the title company releases the funds. This requires agreement from the other party. If they refuse, move to mediation or court.',
      acknowledgeLabel: 'I understand the mutual release process →',
      showIf: (answers) => answers.resolution_path === 'mutual_release',
    },
    {
      id: 'mediation_detail',
      type: 'info',
      prompt:
        'MEDIATION:\nMany TREC contracts require mediation before filing suit — check your contract. A neutral mediator helps both sides negotiate. Cost: $200-500 per party for a half-day session. If mediation fails, you can still file suit.',
      acknowledgeLabel: 'I understand the mediation process →',
      showIf: (answers) => answers.resolution_path === 'mediation',
    },
    {
      id: 'small_claims_detail',
      type: 'info',
      prompt:
        'SMALL CLAIMS (JP COURT):\nIf the earnest money is under $20,000, you can file in Justice of the Peace Court. Filing fee: $35-75. The process is faster and simpler than district court, with relaxed evidence rules.',
      acknowledgeLabel: 'I understand the JP Court option →',
      showIf: (answers) => answers.resolution_path === 'small_claims',
    },
    {
      id: 'district_court_detail',
      type: 'info',
      prompt:
        'DISTRICT COURT:\nFor larger amounts or if you are also seeking additional damages (e.g., specific performance, consequential damages such as inspection costs, appraisal fees, or price difference for a replacement property). Filing fee: $300-400.',
      acknowledgeLabel: 'I understand the District Court option →',
      showIf: (answers) => answers.resolution_path === 'district_court',
    },
  ],
  ...
  generateSummary(answers) {
    ...
    // Resolution path
    if (answers.resolution_path) {
      const pathLabels: Record<string, string> = {
        mutual_release: 'Mutual release (TREC release form)',
        mediation: 'Mediation',
        small_claims: 'Small claims (JP Court)',
        district_court: 'District Court',
      }
      items.push({
        status: 'done',
        text: `Resolution path selected: ${pathLabels[answers.resolution_path]}.`,
      })
    } else if (answers.what_happened === 'mutual_disagreement' || answers.what_happened === 'title_company_holding') {
      items.push({
        status: 'info',
