...
export async function POST(request: NextRequest) {
  try {
    ...
    // Bypasses the task PATCH API because VALID_TRANSITIONS blocks todo→completed directly.
    // Intentionally omits: task_events insert, confidence recompute, autoGenerateDeadlines,
    // and analytics trackEvent — this completion is silent by design.
    ...
    if (welcomeTask && welcomeTask.status === 'todo') {
      ...
      if (!welcomeError) {
        // Re-fetch tasks so the DB trigger's unlock of the next task is reflected in the response.
        const { data: refreshed } = await supabase
          .from('tasks')
          .select()
          .eq('case_id', caseId)
          .order('created_at', { ascending: true })
        if (refreshed) finalTasks = refreshed
      } else {
        console.warn('[cases-api] Failed to auto-complete welcome task:', welcomeError.message)
      }
    }

    return NextResponse.json({ case: newCase, tasks: finalTasks }, { status: 201 })
  } catch {
    return NextResponse.json(
