-- ============================================
-- Update get_case_dashboard to include label, consequence in deadlines
-- ============================================
...
BEGIN
  ...
  -- Upcoming deadlines: next 14 days (now includes label + consequence)
  SELECT COALESCE(jsonb_agg(to_jsonb(d) ORDER BY d.due_at), '[]'::jsonb) INTO v_upcoming_deadlines
  FROM (
    SELECT id, key, due_at, source, rationale, label, consequence
