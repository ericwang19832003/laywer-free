-- =============================================================
-- LANDLORD-TENANT WORKFLOW IMPROVEMENTS
-- =============================================================
-- Adds 7 new depth/educational task_keys to the landlord-tenant workflow:
--   - lt_sb38_awareness (after landlord_tenant_intake, alongside evidence_vault)
--   - lt_federal_property_check (after landlord_tenant_intake, alongside evidence_vault)
--   - lt_retaliation_defense (after lt_eviction_response completed — depth task)
--   - lt_rent_into_registry (after lt_appeal_guide completed — depth task)
--   - lt_post_eviction_rights (after lt_hearing_day, alongside lt_post_judgment)
--   - lt_constructive_eviction (after lt_habitability_checklist completed — depth task)
--   - lt_code_enforcement (after lt_repair_request completed — depth task)
--
-- Steps:
--   1. CREATE OR REPLACE seed_case_tasks() with updated LT section
--   2. CREATE OR REPLACE unlock_next_task() with updated LT transitions
--   3. Backfill existing landlord_tenant cases
-- =============================================================


-- =============================================================
-- STEP 1: CREATE OR REPLACE seed_case_tasks()
-- =============================================================
...
BEGIN
  ...
  -- ========================================
  -- Debt collection defendant cases — 17 tasks (updated)
  -- ========================================
  IF NEW.dispute_type = 'debt_collection' AND NEW.role = 'defendant' THEN
    ...
    INSERT INTO public.tasks (case_id, task_key, title, status)
    VALUES
      (NEW.id, 'debt_defense_intake', 'Tell Us About the Debt', 'locked'),
      (NEW.id, 'debt_court_type_guide', 'Understanding Your Court', 'locked'),
      (NEW.id, 'evidence_vault', 'Organize Your Evidence', 'locked'),
      (NEW.id, 'prepare_debt_validation_letter', 'Draft a Debt Validation Letter', 'locked'),
      (NEW.id, 'debt_pre_answer_settlement', 'Consider Early Settlement', 'locked'),
      (NEW.id, 'prepare_debt_defense_answer', 'Prepare Your Answer', 'locked'),
      (NEW.id, 'debt_standing_challenge', 'Challenge Their Right to Sue', 'locked'),
      (NEW.id, 'debt_file_with_court', 'File With the Court', 'locked'),
      (NEW.id, 'serve_plaintiff', 'Serve the Plaintiff', 'locked'),
      (NEW.id, 'debt_discovery_response', 'Respond to Their Discovery', 'locked'),
      (NEW.id, 'debt_business_records_challenge', 'Challenge Their Evidence', 'locked'),
      (NEW.id, 'debt_hearing_prep', 'Prepare for Your Hearing', 'locked'),
      (NEW.id, 'debt_hearing_day', 'Hearing Day', 'locked'),
      (NEW.id, 'debt_post_judgment', 'After the Ruling', 'locked'),
      (NEW.id, 'debt_exemption_claim', 'Claim Your Protected Property', 'locked'),
      (NEW.id, 'debt_appeal_process', 'How to Appeal', 'locked');
  ...
  -- ========================================
  -- Landlord-tenant cases — 21 tasks (updated: was 16, +7 depth tasks)
  -- ========================================
  IF NEW.dispute_type = 'landlord_tenant' THEN
    ...
    VALUES
      ...
      -- NEW depth/educational tasks (7)
      (NEW.id, 'lt_sb38_awareness', '2026 Eviction Law Changes (SB 38)', 'locked'),
      (NEW.id, 'lt_federal_property_check', 'Is Your Property Federally Backed?', 'locked'),
      (NEW.id, 'lt_retaliation_defense', 'Building Your Retaliation Defense', 'locked'),
      (NEW.id, 'lt_rent_into_registry', 'Paying Rent Into the Court Registry', 'locked'),
      (NEW.id, 'lt_post_eviction_rights', 'Your Rights After Eviction', 'locked'),
      (NEW.id, 'lt_constructive_eviction', 'Constructive Eviction', 'locked'),
      (NEW.id, 'lt_code_enforcement', 'Filing a Code Enforcement Complaint', 'locked');
...
BEGIN
  ...
  -- ========================================
  -- Debt defense chain (updated — 15 transitions)
  -- ========================================

  -- welcome -> debt_defense_intake
  IF NEW.task_key = 'welcome' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_defense_intake' AND status = 'locked';
  END IF;

  -- debt_defense_intake -> debt_court_type_guide (CHANGED: was evidence_vault)
  IF NEW.task_key = 'debt_defense_intake' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_court_type_guide' AND status = 'locked';
  END IF;

  -- debt_court_type_guide -> evidence_vault (NEW)
  IF NEW.task_key = 'debt_court_type_guide' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'evidence_vault' AND status = 'locked';
  END IF;

  -- evidence_vault -> prepare_debt_validation_letter (unchanged)
  IF NEW.task_key = 'evidence_vault' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'prepare_debt_validation_letter' AND status = 'locked';
  END IF;

  -- prepare_debt_validation_letter -> debt_pre_answer_settlement (CHANGED: was prepare_debt_defense_answer)
  IF NEW.task_key = 'prepare_debt_validation_letter' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_pre_answer_settlement' AND status = 'locked';
  END IF;

  -- debt_pre_answer_settlement -> prepare_debt_defense_answer (NEW, completed OR skipped)
  IF NEW.task_key = 'debt_pre_answer_settlement' AND NEW.status IN ('completed', 'skipped') AND OLD.status NOT IN ('completed', 'skipped') THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'prepare_debt_defense_answer' AND status = 'locked';
  END IF;

  -- prepare_debt_defense_answer -> debt_standing_challenge (CHANGED: was debt_file_with_court)
  IF NEW.task_key = 'prepare_debt_defense_answer' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_standing_challenge' AND status = 'locked';
  END IF;

  -- debt_standing_challenge -> debt_file_with_court (NEW, completed OR skipped)
  IF NEW.task_key = 'debt_standing_challenge' AND NEW.status IN ('completed', 'skipped') AND OLD.status NOT IN ('completed', 'skipped') THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_file_with_court' AND status = 'locked';
  END IF;

  -- debt_file_with_court -> serve_plaintiff (unchanged)
  ...
  -- serve_plaintiff -> debt_discovery_response AND debt_business_records_challenge (CHANGED: was debt_hearing_prep)
  IF NEW.task_key = 'serve_plaintiff' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_discovery_response' AND status = 'locked';
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_business_records_challenge' AND status = 'locked';
  END IF;

  -- debt_discovery_response completed/skipped -> unlock debt_hearing_prep (only if business_records_challenge also done)
  IF NEW.task_key = 'debt_discovery_response' AND NEW.status IN ('completed', 'skipped') AND OLD.status NOT IN ('completed', 'skipped') THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.tasks
      WHERE case_id = NEW.case_id
      AND task_key = 'debt_business_records_challenge'
      AND status IN ('locked', 'todo', 'in_progress')
    ) THEN
      UPDATE public.tasks SET status = 'todo', unlocked_at = now()
  ...
  -- debt_business_records_challenge completed/skipped -> unlock debt_hearing_prep (only if discovery_response also done)
  IF NEW.task_key = 'debt_business_records_challenge' AND NEW.status IN ('completed', 'skipped') AND OLD.status NOT IN ('completed', 'skipped') THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.tasks
      WHERE case_id = NEW.case_id
      AND task_key = 'debt_discovery_response'
      AND status IN ('locked', 'todo', 'in_progress')
    ) THEN
      UPDATE public.tasks SET status = 'todo', unlocked_at = now()
      WHERE case_id = NEW.case_id AND task_key = 'debt_hearing_prep' AND status = 'locked';
  ...
  -- debt_hearing_prep -> debt_hearing_day (unchanged)
  ...
  -- debt_hearing_day -> debt_post_judgment (unchanged)
  ...
  -- debt_post_judgment -> debt_exemption_claim AND debt_appeal_process (NEW, parallel)
  IF NEW.task_key = 'debt_post_judgment' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_exemption_claim' AND status = 'locked';
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'debt_appeal_process' AND status = 'locked';
  END IF;

  -- ========================================
  -- Landlord-tenant chain (updated — 15 primary + 7 depth transitions)
  -- ========================================
  ...
  -- landlord_tenant_intake -> evidence_vault + lt_sb38_awareness + lt_federal_property_check (UPDATED: added 2 depth tasks)
  IF NEW.task_key = 'landlord_tenant_intake' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'evidence_vault' AND status = 'locked';
    -- NEW: unlock depth tasks alongside evidence_vault
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'lt_sb38_awareness' AND status = 'locked';
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'lt_federal_property_check' AND status = 'locked';
  END IF;
  ...
  -- lt_hearing_day -> lt_post_judgment + lt_post_eviction_rights (UPDATED: added depth task)
  IF NEW.task_key = 'lt_hearing_day' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    -- NEW: unlock depth task alongside lt_post_judgment
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'lt_post_eviction_rights' AND status = 'locked';
  END IF;

  -- ========================================
  -- LT DEPTH TASK TRIGGERS (4 new blocks)
  -- These trigger on depth tasks seeded by seed_lt_depth_tasks()
  -- ========================================

  -- lt_eviction_response -> lt_retaliation_defense
  IF NEW.task_key = 'lt_eviction_response' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'lt_retaliation_defense' AND status = 'locked';
  END IF;

  -- lt_appeal_guide -> lt_rent_into_registry
  IF NEW.task_key = 'lt_appeal_guide' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'lt_rent_into_registry' AND status = 'locked';
  END IF;

  -- lt_habitability_checklist -> lt_constructive_eviction
  IF NEW.task_key = 'lt_habitability_checklist' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'lt_constructive_eviction' AND status = 'locked';
  END IF;

  -- lt_repair_request -> lt_code_enforcement
  IF NEW.task_key = 'lt_repair_request' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    UPDATE public.tasks SET status = 'todo', unlocked_at = now()
    WHERE case_id = NEW.case_id AND task_key = 'lt_code_enforcement' AND status = 'locked';
  END IF;
  ...
  -- divorce_file_with_court -> divorce_standing_orders
  ...
  -- divorce_serve_respondent -> divorce_response_checkpoint
  ...
  -- divorce_final_orders -> divorce_post_decree
  ...
  -- custody_uccjea_affidavit -> custody_paternity
  ...
  -- custody_paternity -> custody_safety_screening
  ...
  -- custody_serve_respondent -> custody_response_checkpoint
  ...
  -- custody_final_orders -> custody_post_decree
  ...
  -- child_support_ag_option -> child_support_paternity
  ...
  -- child_support_paternity -> child_support_evidence_vault
  ...
  -- child_support_serve_respondent -> child_support_response_checkpoint
  ...
  -- child_support_final_orders -> child_support_post_decree
  ...
  -- visitation_serve_respondent -> visitation_response_checkpoint
  ...
  -- visitation_final_orders -> visitation_post_decree
  ...
  -- spousal_support_eligibility -> spousal_support_evidence_vault
  ...
  -- spousal_support_serve_respondent -> spousal_support_response_checkpoint
  ...
  -- spousal_support_final_orders -> spousal_support_post_decree
  ...
  -- mod_serve_respondent -> mod_response_checkpoint
  ...
  -- mod_final_orders -> mod_post_decree
...
-- =============================================================
-- STEP 3: Backfill existing landlord_tenant cases
-- =============================================================

-- lt_sb38_awareness
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'lt_sb38_awareness', '2026 Eviction Law Changes (SB 38)', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'landlord_tenant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'lt_sb38_awareness');

-- lt_federal_property_check
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'lt_federal_property_check', 'Is Your Property Federally Backed?', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'landlord_tenant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'lt_federal_property_check');

-- lt_retaliation_defense
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'lt_retaliation_defense', 'Building Your Retaliation Defense', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'landlord_tenant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'lt_retaliation_defense');

-- lt_rent_into_registry
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'lt_rent_into_registry', 'Paying Rent Into the Court Registry', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'landlord_tenant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'lt_rent_into_registry');

-- lt_post_eviction_rights
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'lt_post_eviction_rights', 'Your Rights After Eviction', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'landlord_tenant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'lt_post_eviction_rights');

-- lt_constructive_eviction
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'lt_constructive_eviction', 'Constructive Eviction', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'landlord_tenant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'lt_constructive_eviction');

-- lt_code_enforcement
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'lt_code_enforcement', 'Filing a Code Enforcement Complaint', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'landlord_tenant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'lt_code_enforcement');
