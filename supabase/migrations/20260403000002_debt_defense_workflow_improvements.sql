-- =============================================================
-- DEBT DEFENSE WORKFLOW IMPROVEMENTS
-- =============================================================
-- Adds 7 new task_keys to the debt defense workflow:
--   - debt_court_type_guide (after intake, before evidence_vault)
--   - debt_pre_answer_settlement (after validation letter, before answer)
--   - debt_standing_challenge (after answer, before file_with_court)
--   - debt_discovery_response (after serve_plaintiff, parallel)
--   - debt_business_records_challenge (after serve_plaintiff, parallel)
--   - debt_exemption_claim (after post_judgment, parallel)
--   - debt_appeal_process (after post_judgment, parallel)
--
-- Steps:
--   1. CREATE OR REPLACE seed_case_tasks() with updated debt section
--   2. CREATE OR REPLACE unlock_next_task() with updated debt transitions
--   3. Backfill existing debt_collection defendant cases
-- =============================================================
...
-- =============================================================
-- STEP 3: Backfill existing debt_collection defendant cases
-- =============================================================

-- debt_court_type_guide
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'debt_court_type_guide', 'Understanding Your Court', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'debt_collection' AND c.role = 'defendant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'debt_court_type_guide');

-- debt_pre_answer_settlement
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'debt_pre_answer_settlement', 'Consider Early Settlement', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'debt_collection' AND c.role = 'defendant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'debt_pre_answer_settlement');

-- debt_standing_challenge
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'debt_standing_challenge', 'Challenge Their Right to Sue', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'debt_collection' AND c.role = 'defendant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'debt_standing_challenge');

-- debt_discovery_response
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'debt_discovery_response', 'Respond to Their Discovery', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'debt_collection' AND c.role = 'defendant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'debt_discovery_response');

-- debt_business_records_challenge
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'debt_business_records_challenge', 'Challenge Their Evidence', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'debt_collection' AND c.role = 'defendant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'debt_business_records_challenge');

-- debt_exemption_claim
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'debt_exemption_claim', 'Claim Your Protected Property', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'debt_collection' AND c.role = 'defendant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'debt_exemption_claim');

-- debt_appeal_process
INSERT INTO public.tasks (case_id, task_key, title, status)
SELECT c.id, 'debt_appeal_process', 'How to Appeal', 'locked'
FROM public.cases c
WHERE c.dispute_type = 'debt_collection' AND c.role = 'defendant'
AND NOT EXISTS (SELECT 1 FROM public.tasks WHERE case_id = c.id AND task_key = 'debt_appeal_process');
