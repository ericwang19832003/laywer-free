-- When pi_settlement_negotiation is SKIPPED, unlock prepare_pi_petition
-- so users can bypass pre-litigation negotiation and go straight to filing.
--
-- The existing trigger only handles 'completed' with conditional branching
-- based on guided answers. Skipping means the user explicitly chose to
-- skip negotiation, so we always unlock the petition step.
--
-- This replaces the entire unlock_next_task function to add the new handler.
-- All existing chains are preserved exactly as-is.
...
BEGIN
  ...
  IF NEW.task_key = 'pi_settlement_negotiation' AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    IF v_settlement_reached = 'no' AND v_want_to_file_suit = 'yes' THEN
      ...
      ELSE
        -- Not filed yet: unlock petition prep
        UPDATE public.tasks SET status = 'todo', unlocked_at = now()
        WHERE case_id = NEW.case_id AND task_key = 'prepare_pi_petition' AND status = 'locked';
      END IF;
    ELSE
  ...
  -- NEW: PI: pi_settlement_negotiation SKIPPED -> unlock prepare_pi_petition
  -- User chose to skip negotiation entirely and go straight to filing
  ...
  -- Civil: preservation_letter -> prepare_filing (handles skipped too)
  IF NEW.task_key = 'preservation_letter' AND NEW.status IN ('completed', 'skipped') AND OLD.status NOT IN ('completed', 'skipped') THEN
    UPDATE public.tasks
    SET status = 'todo', unlocked_at = now()
