...
CREATE OR REPLACE FUNCTION public.create_case_atomic(
  ...
  -- Sub-type params (only one should be non-null at a time)
...
BEGIN
  ...
  -- Validate business sub-type is provided for business cases
  ...
  -- Insert sub-type detail (only one will match)
