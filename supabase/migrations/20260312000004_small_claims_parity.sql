-- =============================================================
-- SMALL CLAIMS NAMING PARITY
-- =============================================================
-- Renames 6 generic/shared task_keys to sc_* variants so small
-- claims tasks are fully namespaced like every other workflow.
--
-- Rename map:
--   evidence_vault        → sc_evidence_vault
--   prepare_demand_letter → sc_demand_letter
--   file_with_court       → sc_file_with_court
--   serve_defendant       → sc_serve_defendant
--   prepare_for_hearing   → sc_prepare_for_hearing
--   hearing_day           → sc_hearing_day
--
-- Steps:
--   1. Rename existing SC task_keys in the tasks table
--   2. CREATE OR REPLACE seed_case_tasks()
--   3. CREATE OR REPLACE unlock_next_task()
-- =============================================================


-- =============================================================
-- STEP 1: Rename existing SC task_keys
-- =============================================================

UPDATE public.tasks SET task_key = 'sc_evidence_vault'
WHERE task_key = 'evidence_vault'
  AND case_id IN (SELECT id FROM public.cases WHERE dispute_type = 'small_claims');

UPDATE public.tasks SET task_key = 'sc_demand_letter'
WHERE task_key = 'prepare_demand_letter'
  AND case_id IN (SELECT id FROM public.cases WHERE dispute_type = 'small_claims');

UPDATE public.tasks SET task_key = 'sc_file_with_court'
WHERE task_key = 'file_with_court'
  AND case_id IN (SELECT id FROM public.cases WHERE dispute_type = 'small_claims');

UPDATE public.tasks SET task_key = 'sc_serve_defendant'
WHERE task_key = 'serve_defendant'
  AND case_id IN (SELECT id FROM public.cases WHERE dispute_type = 'small_claims');

UPDATE public.tasks SET task_key = 'sc_prepare_for_hearing'
WHERE task_key = 'prepare_for_hearing'
  AND case_id IN (SELECT id FROM public.cases WHERE dispute_type = 'small_claims');

UPDATE public.tasks SET task_key = 'sc_hearing_day'
